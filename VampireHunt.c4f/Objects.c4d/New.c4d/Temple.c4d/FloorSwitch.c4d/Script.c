...
local iYPos;
local idSpezial; // nur die Items mit der passenden ID werden gezählt
local iNeededWeight;
local idSuper; // Eine super ID, die den Schalter mit einem Objekt bedient
local iYPosExtra; // Zusätzliches Runterfahren
...
protected func Timer10()
  ...
  var iDesiredY;
  // Gewicht prüfen, dabei entweder eigenen Wert verwenden oder Standartwert
  if(iNeededWeight) { if (iWgt >= iNeededWeight) iDesiredY = _FLS_DownPos+iYPosExtra; }
  else if (iWgt >= _FLS_NeededWeight) iDesiredY = _FLS_DownPos+iYPosExtra;
