...
public func StrikeSound()     { return Sound("SwordHitd*"); }
public func GetStrikeDamage(){ return 3; }
public func GetStrikeTime()  { return 1+Random(2); }
public func GetCausedPain()  { return  6+Random(4); }
public func GetBloodLoss()   { return Min(Random(5)-3,0);} //1/5

public func ControlLeft(object pController){ControlRight(pController,1);}
...
private func ReturnControl(object pController,int i){fBlock = 0; Schedule(Format("ControlDigDouble(Object(%d))",ObjectNumber(pController)),i,1,this);}
...
private func FxRunTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
    ...
    if(Distance(GetX(pTarget),GetY(pTarget),GetX(pEne),GetY(pEne)) < 5 && EffectVar(2,pTarget,iEffectNumber) == true)
    {
        ...
        pEne->Pain(30+EffectVar(1,pTarget,iEffectNumber));
    ...
    else if(iEffectTime > 120 && EffectVar(2,pTarget,iEffectNumber) == true)
    {
        SetPhysical("Walk",GetPhysical("Walk",1,pTarget)*3/5,PHYS_Temporary,pTarget);
        EffectVar(2,pTarget,iEffectNumber) = iEffectTime;
        SetComDir(COMD_None,pTarget);
