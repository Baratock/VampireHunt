...
private func GetBowOutOld(pClonk, &iOutX, &iOutY, &iOutR, &iOutXDir, &iOutYDir, &iOutRDir)
{
  ...
  iOutR = iOutX * 13;
  // XDir
  iOutXDir = iOutDir*iOutPhase*2*3;
  // YDir
...
{
  // Zufallsabweichung
  if(fDeviate) { iAngle += Random(9) - 5; }
  // Austrittsgeschwindigkeit ermitteln 
  iOutXDir = Sin(iAngle, 110); 
  iOutYDir = -Cos(iAngle, 110); 
  // Austrittsposition... 
  iOutX = Sin(iAngle, 25); 
  iOutY = -Cos(iAngle, 25); 
  // Pfeildrehung 
  iOutR = iAngle; 
  if(iAngle > 0) iOutRDir = 1; else iOutRDir = -5;
  // Fertig 
  return(1);
} 
...
public func Fire()
{
  ...
  pArrow->SetOwner(Contained()->GetOwner());
    // Pfeil abfeuern
  Exit(pArrow, iOutX,iOutY,iOutR, 0,0,iOutRDir )+4;
...
public func FireAngle(iAngle)
{
  ...
  if (!pArrow) return();
  // Beim Träger ggf. Bogen wieder auswählen
  ShiftContents(Contained(), 0, BOW2);
  // Austrittsgeschwindigkeit ermitteln
  var iOutX, iOutY, iOutR, iOutXDir, iOutYDir, iOutRDir;
  if (!pClonk->~GetBowOut(this(), iAngle, true, iOutX, iOutY, iOutR, iOutXDir, iOutYDir, iOutRDir))
