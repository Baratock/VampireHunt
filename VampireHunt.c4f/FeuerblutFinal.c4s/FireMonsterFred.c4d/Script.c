...
private func Activity()
{
  ...
  // Nichts machen
  if (Random(10) || (GetAction() ne "Walk" && GetAction() ne "Swim")) return(1);
  ...
  // Feuer speien
  if (GetAction() eq "Walk")
   if (!Random(2)) return(StartFlame());

  // Umsehen
  if (GetAction() eq "Walk")
   if (!Random(10)) return(SetAction("LookUp"));

  // Umdrehen
  if (Random(1)) return(TurnRight());
  return(TurnLeft());
}
