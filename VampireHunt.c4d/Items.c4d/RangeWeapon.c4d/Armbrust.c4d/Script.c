...
public func ControlUp(object pClonk)               // hoch zielen
{
	AimUp(pClonk, 1, "ControlConf", CRBW);
}

public func ControlDown(object pClonk)             // runter zielen
{
...
public func ActualizePhase(object pClonk)
{
	...
	if (pClonk->GetAction() == "LoadCrossbow")
		iColor = RGB(255);
}
...
public func ControlSpecial(object pClonk)
{
	// Träger Zielaktion
	if (IsAiming() || IsLoading())
	{
...
public func ControlRightDouble(object pClonk)
{
...
public func ControlLeftDouble(object pClonk)
{
	// Zielaktion abbrechen
	if (IsAiming() || IsLoading())
	{
...
/* Munitionssuche */
private func GetMunition(int iIndex)
{
	if (!Contained())
		return 0;
...
private func CheckMunitionID(object pMuni)
{
	...
	for (var pObj in FindObjects(Find_Container(Contained()), Find_Func("IsBoltPack"))) 
	{
		...
		if (idFound == GetID(pMuni) && LocalN("poision", pMuni) == LocalN("poision", pObj))
			return 1;
	}
}
...
public func GetCrossbowOut(pBow, int iAngle, fDeviate, & iOutX, & iOutY, & iOutR, & iOutXDir, & iOutYDir, & iOutRDir)
{
	// Austrittsgeschwindigkeit ermitteln 
