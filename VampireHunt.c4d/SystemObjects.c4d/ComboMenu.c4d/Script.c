...
private func CreateSpellSymbol(id idSpell, string szAction, int iPosX, int iPosY, fSmall, & riMinX, & riMaxX, & riMinY, & riMaxY)
{
	// Symbol erzeugen
...
public func GetAbilitys(object pTarget)
{
	...
	for (var i2 = 0; GetDefinition(i2); i2++)
	{
		if (GetDefinition(i2)->~IsAbility() && GetDefinition(i2)->~CanUse(pCallBack))
		{
			aAbi[i] = GetDefinition(i2);
			i++;
		}
	}
	return aAbi;
...
private func CheckSpells()
{
	...
	while (i--)
	{
		...
		iSymCount = Var(iKey + 6)++;
		iMaxSymCount = Var(iKey);
		if (!iSymCount)
		...
		var iRadius = 70 + iRow * 40, iSymAngleDiff = iRadius / 5;
	...
	if (iXMin + GetX() < 17)
		...
		if (iXMax + GetX() > LandscapeWidth() - 17)
			iMoveX = -17 - iXMax - GetX() + LandscapeWidth();
	if (iYMin + GetY() < 17)
		iMoveY = 17 - iYMin - GetY();
	else
		if (iYMax + GetY() > LandscapeHeight() - 17)
...
private func MoveSpellSymbols(int iByX, int iByY)
{
...
{
	return DoCastControl("L");
}
protected func ControlRight()
{
	return DoCastControl("R");
}
protected func ControlUp()
{
	return DoCastControl("5");
}
protected func ControlDown()
{
	return DoCastControl("2");
}
protected func ControlThrow()
{
	return DoCastControl("4");
}
protected func ControlDig()
{
	return DoCastControl("6");
}
...
protected func DoCastControl(string szControl)
{
...
// ComString2Index
private func GetComIndex(string szCom)
{
	return GetChar(szCom) - 49; // GetChar("1")==49
}

// ComChar2Index
private func GetComCharIndex(int cCom)
{
